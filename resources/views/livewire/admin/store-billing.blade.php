                ...
                <tbody>
                    ...
                    <tr wire:key="cart-{{ $id }}">
                        ...
                        <td class="text-center">
                            <!-- Editable Unit Price -->
                            <div class="input-group input-group-sm mx-auto" style="width: 160px;">
                                <span class="input-group-text">Rs.</span>
                                <input type="number" 
                                       class="form-control form-control-sm text-center"
                                       wire:change="updateUnitPrice({{ $id }}, $event.target.value)"
                                       wire:model.live="prices.{{ $id }}"
                                       min="0" 
                                       step="0.01"
                                       value="{{ $prices[$id] ?? $item['price'] }}">
                            </div>
                            @if(isset($prices[$id]) && $prices[$id] != $item['original_price'])
                            <small class="text-muted d-block" style="font-size: 10px;">
                                Original: Rs.{{ number_format($item['original_price'], 2) }}
                            </small>
                            @endif
                        </td>
                        <td class="text-center">
                        ...
                        <td class="text-center">
                            <div class="input-group input-group-sm mx-auto" style="width: 120px;">
                                ...
                                <input type="number" class="form-control form-control-sm"
                                       ...
                                       max="{{ $prices[$id] ?? $item['price'] }}" step="0.01"
                                       wire:change="updateDiscount({{ $id }}, $event.target.value)">
                            </div>
                        </td>
                        <td class="text-center">
                            <span class="fw-bold text-dark">
                                Rs.{{ number_format((($prices[$id] ?? $item['price']) * $quantities[$id]) - ($discounts[$id] ?? 0) * $quantities[$id], 2) }}
                            </span>
                        </td>
